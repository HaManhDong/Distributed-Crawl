...
class HuffingtonPostSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if news_title is not None and content is not None and news_time is not None and news_type is not None:
            ...
            HuffingtonPostSpider.db_session.add(news_page_data)
            HuffingtonPostSpider.db_session.commit()

        HuffingtonPostSpider.next_urls += self.get_next_link_list(response)

        # write to file
        target = open(self.name, 'w')
        for url in HuffingtonPostSpider.next_urls:
            target.write(url)
            target.write('\n')
        target.close()
